...
Đề tài: Ứng dụng mô hình nhận dạng hành vi con người hỗ trợ giám sát thông minh tại viện dưỡng lão
...
nn_budget = None
nms_max_overlap = 1.0
complane = pyglet.media.load('hehe.mp3')
#deep_sort
    ...
    def humans_to_skelsInfo(humans, joint_choose, action_type="None"):
        ...
        for human in humans:
            ...
            for i, body_part in human.body_parts.items(): # iterate dict
                ...
                lx.append(int(body_part.x * 1280 + 0.5)) # 960: width of frame (chon tuy theo camera)
                skeleton[1+2*idx+1]=body_part.y
                ly.append(int(body_part.y * 1000 + 0.5)) # 720: height of frame (chon tuy theo camera)
